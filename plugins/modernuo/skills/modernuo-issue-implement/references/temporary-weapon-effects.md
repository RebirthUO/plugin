...
When official sources establish behavior but omit exact constants (for example, duration scaling by selected spell level), choose a deterministic table explicitly in code and tests. Record the policy and its cap. Do not copy an emulator's skill-only formula and call it canonical if it conflicts with the issue's source evidence.
