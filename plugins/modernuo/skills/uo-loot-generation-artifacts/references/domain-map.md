# Loot Generation and Artifacts Domain Map

Detailed loot/chest/artifact architecture, tables, examples, and historical observations deferred from the skill entrypoint. Verify probabilities and reachability in current source.
...
For named artifacts or events, research current official sources and the
configured repository at use time. Do not reuse stored item constants,
community infobox values, emulator defaults, or event-version assumptions.
