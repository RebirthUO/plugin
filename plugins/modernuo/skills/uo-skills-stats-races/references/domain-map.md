# Skills, Stats, and Races Domain Map

Detailed taxonomy/cap/race tables, examples, and historical observations deferred from the skill entrypoint. Verify roster and gain formulas in active data/code and target-era sources.
...
- `www.uoguide.com/Skills`, `/Stat`, and `/Races` are community discovery
  references only. They may supply search terms or historical leads but cannot
  establish official skill counts, stat formulas, caps, or racial abilities.
  Verify those claims through `uo-official-evidence`.
