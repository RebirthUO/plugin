...
contract="\n".join(path.read_text(encoding="utf-8") for path in [Path(__file__).parents[1]/"SKILL.md",Path(__file__).parents[1]/"references/authoring-contract.md"]).casefold()
for token in ["canonical fallback field order", "template_optional_reason", "never create from a stale packet", "label_selection", "ambiguous_label", "issue-specific rationale"]: assert token in contract
