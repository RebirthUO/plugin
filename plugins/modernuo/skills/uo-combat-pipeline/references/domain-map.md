# Combat Pipeline Domain Map

Detailed architecture, formulas, tables, examples, and historical observations deferred from the skill entrypoint. Verify formulas and hooks against the active branch before editing.
...
The following rows describe the local implementation vocabulary. The archived
Stratics AoS Combat spotlight is community discovery material, not an official
OSI/EA source. Verify era-scoped gameplay claims through
`uo-official-evidence` before using this table as an expected-behavior contract:
...
For Honor/Perfection, establish era-scoped official gain, cap, luck, damage, and
kill-restore behavior first, then inspect the configured repository's current
formulas. Do not retain branch-specific formulas or examples in this reference.
...
For Bushido and Ninjitsu, establish official behavior through
`uo-official-evidence`, load `uo-samurai-empire-skills`, and design current
repository tests through `modernuo-test-workflow`. Do not reuse issue-specific
fixtures or constants.
