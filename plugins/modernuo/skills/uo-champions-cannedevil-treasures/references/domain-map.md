# Champions and Treasures Domain Map

Detailed architecture, event tables, examples, and historical observations deferred from the skill entrypoint. Verify event availability and reward rules against the active branch and sources.
...
- `www.uoguide.com/Champion_spawn` and `/Star_Room` are community discovery
  references only. Verify candle scaling, rewards, skull altars, and Harrower
  behavior through official sources.
