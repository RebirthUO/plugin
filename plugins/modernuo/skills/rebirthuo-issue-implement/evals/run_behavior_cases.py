...
contract="\n".join(path.read_text(encoding="utf-8") for path in [Path(__file__).parents[1]/"SKILL.md",Path(__file__).parents[1]/"references/implementation-workflow.md"]).casefold()
for token in ["complete", "ready", "research"]: assert token in contract
