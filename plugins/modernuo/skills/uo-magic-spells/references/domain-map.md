# Magic and Spells Domain Map

Detailed school tables, lifecycle notes, examples, and historical observations deferred from the skill entrypoint. Verify spell registrations, formulas, and IDs in the active branch.
...
9. **`ITargetingSpell<Mobile>` also matches point-target spells through contravariance.** `ITargetingSpell<in T>` is contravariant, so a class implementing `ITargetingSpell<IPoint3D>` (fields, summons, ground-target AoE such as `BladeSpiritsSpell`, `MeteorSwarmSpell`, `FireFieldSpell`) satisfies `spell is ITargetingSpell<Mobile>`. For single-mobile damage filters, require `spell is ITargetingSpell<Mobile> && spell is not ITargetingSpell<IPoint3D>` and add tests for poison, fields, summons, and AoE exclusions.
10. **Bypassing `CheckCast` on custom mounts.** `Mounted` blocks most spells, but some allow mounts (Magery with `SpellChanneling` on the weapon, some ML spells). The check is per-spell.
