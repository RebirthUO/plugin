# World, Facets, and Regions Domain Map

Detailed map/sector/region/travel/spawn tables, examples, and historical observations deferred from the skill entrypoint. Verify local map support, hook signatures, and JSON schema in the active branch.
...
For faction-related region work, inspect the configured repository's current
faction registration, notoriety handlers, vendors/traps, party integration,
profile gates, and tests. Historical folders or matrices do not prove live
reachability.
