# Samurai Empire Skills Domain Map

Detailed Bushido/Ninjitsu ability and Animal Form tables, product-impact prompts, and historical observations deferred from the skill entrypoint. Verify current and launch-era differences before parity work.
...
- For implementation work, load `uo-official-evidence`,
  `uo-skills-stats-races`, and the relevant ModernUO implementation skill
  before editing.
...
- Do not claim a configured-repository feature is live because a class exists.
  Check registration, expansion gates, maps, trainers, books, items, spawns,
  and tests.
