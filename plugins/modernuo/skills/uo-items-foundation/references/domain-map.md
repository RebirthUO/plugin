# Items Foundation Domain Map

Detailed entity anatomy, hierarchy, examples, and historical observations deferred from the skill entrypoint. Verify APIs and generator conventions in the active branch.

## Overview

The Ultima Online item system in a ModernUO-based repository is a layered
entity model. Verify the current engine/content project split before relying on
example paths or ownership.
...
- `references/analyzing-modernuo-subsystems.md` - reading recipe for turning a
  ModernUO-based subsystem into a class-level skill.
