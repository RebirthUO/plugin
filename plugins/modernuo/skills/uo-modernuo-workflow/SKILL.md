...
- When syncing the ModernUO plugin to Hermes, scan the entire active profile skills tree, not only `software-development`. Select UO/ModernUO/RebirthUO-themed skills by name/frontmatter trigger, copy whole skill directories including `references/`, `templates/`, and `scripts/`, remove plugin extras that are not selected, and verify exact file equality plus source/plugin skill counts after sync. Do not hand-edit synced plugin copies.
...
- For source-first Broadsword UO vs ModernUO GameDocs / atomic parity documentation pilots, use `references/game-docs-atomic-parity-pilot.md`: scaffold shared Obsidian taxonomy first, extract `uo.com` facts before code search, map atoms to exact code evidence, keep RebirthUO as an overlay, and use conservative statuses (`Unknown` before unreconciled `Different`; `Partial` when only part of an atom is verified).
